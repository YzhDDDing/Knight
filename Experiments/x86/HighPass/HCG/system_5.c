...
void system_5_Update(struct system_5_state* self, float signal1[32], float signal2[32], float signal3[32], float signal_o1[32], float signal_o2[32], float signal_o3[32]) {
    ...
    for (batchIndex = 0;batchIndex < 32;batchIndex += 4) {
        __m128 HighPass_S1_signal_o1_Batch = _mm_load_ps(HighPass_S1_signal_o1 + batchIndex);
        __m128 HighPass_S2_signal_o2_Batch = _mm_load_ps(HighPass_S2_signal_o2 + batchIndex);
        __m128 HighPass_S3_signal_o3_Batch = _mm_load_ps(HighPass_S3_signal_o3 + batchIndex);
        _mm_store_ps(signal_o1 + batchIndex, HighPass_S1_signal_o1_Batch);
        _mm_store_ps(signal_o2 + batchIndex, HighPass_S2_signal_o2_Batch);
        _mm_store_ps(signal_o3 + batchIndex, HighPass_S3_signal_o3_Batch);
    }
}