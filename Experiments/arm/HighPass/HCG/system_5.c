...
void system_5_Update(struct system_5_state* self, float signal1[32], float signal2[32], float signal3[32], float signal_o1[32], float signal_o2[32], float signal_o3[32]) {
    ...
    for (batchIndex = 0;batchIndex < 32;batchIndex += 4) {
        float32x4_t HighPass_S1_signal_o1_Batch = vld1q_f32(HighPass_S1_signal_o1 + batchIndex);
        float32x4_t HighPass_S2_signal_o2_Batch = vld1q_f32(HighPass_S2_signal_o2 + batchIndex);
        float32x4_t HighPass_S3_signal_o3_Batch = vld1q_f32(HighPass_S3_signal_o3 + batchIndex);
        vst1q_f32(signal_o1 + batchIndex, HighPass_S1_signal_o1_Batch);
        vst1q_f32(signal_o2 + batchIndex, HighPass_S2_signal_o2_Batch);
        vst1q_f32(signal_o3 + batchIndex, HighPass_S3_signal_o3_Batch);
    }
}